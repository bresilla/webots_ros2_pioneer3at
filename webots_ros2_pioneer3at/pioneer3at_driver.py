...
class Pioneer3atDriver:
    def step(self):
        rclpy.spin_once(self.__node, timeout_sec=0)

    def init(self, webots_node, properties):
        self.__robot = webots_node.robot
        self.FRONT_WHEEL_RADIUS = 0.11
        self.WHEEL_SEPARATION = 0.4

        self.left_front_wheel = self.__robot.getDevice("front left wheel")
        self.left_front_wheel.setPosition(float('inf'))
        self.left_front_wheel.setVelocity(0.0)
        self.right_front_wheel = self.__robot.getDevice("front right wheel")
        self.right_front_wheel.setPosition(float('inf'))
        self.right_front_wheel.setVelocity(0.0)
        self.left_rear_wheel = self.__robot.getDevice("back left wheel")
        self.left_rear_wheel.setPosition(float('inf'))
        self.left_rear_wheel.setVelocity(0.0)
        self.right_rear_wheel = self.__robot.getDevice("back right wheel")
        ...
        self.__node.create_subscription(NavSatFix, '/Pioneer3at/gps_front', self.main_callback, 10)
        self.__node.create_subscription(NavSatFix, '/Pioneer3at/gps_back', self.aux_callback, 10)
